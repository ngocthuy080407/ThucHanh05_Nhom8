# NHÓM 8
# Bài Thực Hành 05 - Tạo, quản lý và chia sẻ nội dung số 

## 📝 Mô tả nội dung
Mục tiêu thực hành:
- Tìm kiếm, lọc, đánh giá và quản lý dữ liệu số đơn giản trong bối cảnh học tập.
- Giao tiếp, chia sẻ thông tin và hợp tác nhóm trực tuyến với quy tắc ứng xử cơ bản.
- Tạo, chỉnh sửa nội dung số (văn bản, đa phương tiện, mã nguồn) và áp dụng bản quyền.
- Sử dụng và đánh giá công cụ AI đạo đức, minh bạch trong ứng dụng đơn giản.


### Danh sách các tệp tin:
- **NV 1.1:** [`.pdf`](https://github.com/ngocthuy080407/ThucHanh05_Nhom8/blob/main/NV%201.1.pdf)
- **NV 2.1:** [`.png`](https://github.com/ngocthuy080407/ThucHanh05_Nhom8/blob/main/NV%202.1.png)
- **NV 2.2:** [`.mp4`](https://github.com/ngocthuy080407/ThucHanh05_Nhom8/blob/main/NV%202.2.mp4)
- **NV 3.1:** [`.html`](https://github.com/ngocthuy080407/ThucHanh05_Nhom8/blob/main/NV%203.1/NV3.1.html), [`.png`](https://github.com/ngocthuy080407/ThucHanh05_Nhom8/blob/main/NV%203.1/infographic.png), [`.mp4 `](https://github.com/ngocthuy080407/ThucHanh05_Nhom8/blob/main/NV%203.1/video.mp4)

---

## Trích dẫn nguồn 
1. **Nguồn cho NV 1.2:** Lấy từ [`.pdf `](https://github.com/ngocthuy080407/ThucHanh05_Nhom8/blob/main/NV%201.1.pdf)
2. **Hình ảnh:** Lấy từ Unsplash - Giấy phép CC0
3. **Video:** Lấy từ Pexel -  Giấy phép CC0

---

## Giấy phép 
Các sản phẩm này được cấp phép theo **Creative Commons Attribution 4.0 International (CC BY)**.
https://creativecommons.org/licenses/by/4.0/
---

